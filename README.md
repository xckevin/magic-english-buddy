...
- L1 的 20 篇课程、英文听题和课文单词提供 Kokoro 预生成音频，随 PWA 缓存；其他内容使用设备英语语音。
- 离线词典实际为 744 个词条。按内置课文单词出现次数统计，可查询覆盖率约 89%，含词形还原；不是完整英语词典。
- 二维码目前用于导出本机学习摘要；扫码导入、班级汇总和 PDF 证书尚未接入当前页面。
- 当前产品使用本机共享学习档案，不支持多个孩子独立切换进度。
...
- **物理证书（待接入）**：仓库保留了 PNG 证书组件，当前页面尚无证书导出入口
