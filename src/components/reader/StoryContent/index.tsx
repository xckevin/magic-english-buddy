...
export const StoryContent = memo<StoryContentProps>(
  ({
    ...
    const getGlobalWordIndex = useCallback(
      (paragraphIndex: number, localWordIndex: number) => {
        let globalIndex = 0;
    ...
    const handleWordClick = useCallback(
      (word: string, localIndex: number, paragraphIndex: number) => {
            ...
            >
              ...
              <p className={styles.englishText}>
                {paragraph.words.map((wordData, localIndex) => {
                  ...
                  const isLearned = learnedWords.has(
                    wordData.word.toLowerCase().replace(/[.,!?]/g, '')
                  );

                  return (
                    <Fragment key={`${paragraph.paragraphId}-${localIndex}`}>
                      <WordHighlight
                        word={wordData.word}
                        index={localIndex}
